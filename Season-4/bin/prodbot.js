...
async function handleAgenticWorkflow(ticker, rl) {
    ...
    // Write summary to sandbox, then back it up
    const sandboxFile = path.join(SANDBOX_DIR, summaryFile);
    fs.writeFileSync(sandboxFile, summaryContent);

    console.log(chalk.hex("#F0A030")("  ☁️  Cloud Backup → backup(" + summaryFile + ")..."));
    await sleep(400);
    let backupInfo;
    try {
        const raw = cloud.tools.backup.run(path.join("prodbot-activities", summaryFile));
        backupInfo = raw.error || raw.result;
